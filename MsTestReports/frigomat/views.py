...
from .serializers import FrigomatReportSerializer, UserSerializer
...
class ReportsListView(generic.ListView):
    template_name = 'frigomat/reportsList.html'
...
class PDFReportView(generic.DetailView):
    ...
    template_name = 'frigomat/pdf_report.html'

    def get_object(self, pk):
        try:
            return self.model.objects.get(pk=pk)
        except FrigomatReport.DoesNotExist:
            raise Http404

    def get(self, request, pk, *args, **kwargs):
        template = get_template('frigomat/pdf_report.html')
...
class ReportListApiView(generics.ListCreateAPIView):
    queryset = FrigomatReport.objects.all()
    serializer_class = FrigomatReportSerializer
